防止内存泄漏应该注意的几点:
(补)
在使用context的时候,也要小心内存泄漏,防止内存泄漏,注意几点:
1. 不要让生命周期长的对象引用ActivityContext ,即要保证引用的activity的对象要与activity本身生命周期的是一样的
2. 对于生命周期长的对象,可以使用application conntext
3. 避免非静态的内部类,尽量使用静态类,避免生命周期问题,注意生命周期问题,注意内部类对外部对象引用导致生命周期变化

注:applicationConntext和ActivityContext 区别?
